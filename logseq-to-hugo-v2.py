...
def filter_private_blocks(content):
    """
    从内容中过滤掉包含 #private 标签的 block。
    Logseq 中每个 block 是一行，通过缩进区分父子关系。
    如果一个 block 包含 #private，则删除这个 block 及其所有子 blocks。
    """
    lines = content.split('\n')
    result_lines = []
    i = 0
...
def convert_logseq_to_hugo(logseq_file_path, hugo_post_dir):
    ...
    # 过滤掉包含 #private 标签的 blocks
    filtered_content = filter_private_blocks(content)
...
"""
    ...
    try:
        with open(hugo_file_path, "w", encoding="utf-8") as hugo_file:
            ...
            hugo_file.write(filtered_content)
